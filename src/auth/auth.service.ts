...
export class AuthService {
  ...
  async register(dtoUser: CreateUserDto) {
    ...
    const userObj = await this.userService.createUser({
      ...dtoUser,
  ...
  async refreshToken(headers) {
    ...
    const tObj = this.jwtService.verify(rToken, {
      secret: process.env.SECRET_KEY,
      ignoreExpiration: true,
    });

    const userDb = await this.userService.getUserByEmail(tObj.email);
    const userObj = this.jwtService.verify(userDb.rt, {
      secret: process.env.SECRET_KEY,
      ignoreExpiration: true,
    });
  ...
  async setNewPassword(dto: SetNewPasswordDto) {
    ...
    return await this.userService.setNewPassword({
      ...dto,
      password: hashPassword,
    });
  }
